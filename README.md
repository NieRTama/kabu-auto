...
kabuステーションのログインは2段階認証があり自動化できない・すべきでないため、
**「ログインされるまで静かに待つ」**ことで自動起動を成立させる。
...
- 起動時に接続できないと、待機に入り **通知先（Discord等）へ「ログイン待ち」を送る**。
  利用者はその通知を見てkabuステーションにログインすればよい。
- ログインが完了して接続できた時点で「接続しました」を通知し、そのまま通常の起動シーケンス
  （注文同期 → プリフライト → スケジューラ起動）へ進む。
...
を持つ）。そのため**毎営業日の朝に1回、手動ログインが必要**になる。
一度ログインすればその日は有効なので、取引のたびにログインする必要はない。
...
つまり朝の運用は「**Discord通知を見てkabuステーションにログインするだけ**」で済み、
以降の復帰操作は要らない。

**kabuステーション自体の自動起動**は、Windows のスタートアップに登録しておく
（`HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run` の `kabuStation`）。
これで OS 起動時にログイン画面までは自動で開くため、朝の操作は
