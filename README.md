...
> **LAN内の他端末から使う場合**: `dashboard.host` を `0.0.0.0` にすると認証が有効になります。
> 起動時にコンソールへ `http://<このPCのIP>:8080/?token=...` が表示されるので、初回はそのURLを
> ブラウザで開いてください（トークンがCookieに保存され、以後はトークン不要）。
...
> 認証が有効な場合（`api_token` 設定時、または `host` が localhost 以外）、各リクエストに
> `-H "X-API-Token: <token>"` が必要です（ローカル既定では不要）。
...
- ログは `log/` フォルダに格納され、1日単位で区切られ、ファイル名末尾に日付が付きます（例: `log/kabu_auto_2026-06-18.log`）。既定では15日周期で古いログが自動削除されます（`logging.retention` で変更可）。
