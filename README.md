...
監視・取引対象の銘柄は `config.yaml` ではなく `watchlist.json` で管理する。
...
起動後はダッシュボードの「🔖 ウォッチリスト管理」カードから銘柄コード・会社名を
入力してGUIで追加・削除できる（プロセス再起動なしで次回のスケジュールジョブから反映される）。
...
| POST | `/api/watchlist` | ウォッチリストに銘柄を追加（`{code, name}`） |
| DELETE | `/api/watchlist/{code}` | ウォッチリストから銘柄を削除 |
...
- `watchlist.json`（ウォッチリスト）はダッシュボードのGUIから追加・削除すると即座にファイルへ反映され、**再起動不要**で次回のスケジュールジョブ（データ更新・シグナルスキャン等）から有効になります。
