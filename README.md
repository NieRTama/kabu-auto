...
| リスク管理 | 損切り・最大保有銘柄数・セクター集中制限・1日注文数上限 |
...
| 09:00 | 市場オープン・WebSocket 接続・価格監視開始 |
| 09:05 | 前日 OHLCV データ取得・指標計算・シグナル生成 |
| 15:30 | 市場クローズ・翌日スクリーニング |
| 15:40 | SQLite 日次バックアップ |
...
| POST | `/api/status` | システム状態更新 |
| GET | `/api/positions` | 現在ポジション（含み損益付き） |
| GET | `/api/trades` | 取引履歴 |
| GET | `/api/signals` | シグナル履歴 |
| GET | `/api/pnl/summary` | 損益サマリ（基本） |
| GET | `/api/pnl/enhanced_summary` | 損益サマリ（MTD/YTD/Sharpe/DD） |
| GET | `/api/pnl/daily` | 日次損益リスト（棒グラフ用） |
| POST | `/api/emergency/close_all` | 緊急全ポジション決済 |
| POST | `/api/ml/retrain` | MLモデル手動再学習トリガー |
| GET | `/api/ml/metrics` | モデル精度履歴 |
| POST | `/api/backtest/run` | バックテスト実行 |
| GET | `/api/backtest/results` | バックテスト結果一覧 |
| GET | `/api/backtest/results/{run_id}` | バックテスト結果詳細 |
| GET | `/api/backtest/results/{run_id}/trades` | バックテスト個別トレード |
