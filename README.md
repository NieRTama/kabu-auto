...
> **既知の制約（2026-09-13時点）**: 2段階認証入力は対話デスクトップを必要とするため、
> kabu-auto本体を起動するタスクスケジューラのログオン種別が `S4U`（対話セッション不要）の
> ままだと自動ログイン自体が失敗しうる。実運用でログイン失敗が続く場合は、まず
> タスクスケジューラの設定を `InteractiveToken` に変更することを疑うこと
> （詳細は `config.yaml` の `runtime.broker_full_login_enabled` 周辺のコメント参照）。
