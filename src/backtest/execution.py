...
class VolumeBudget:
    ...
    **「1営業日」の基準日を呼び出し側が揃えること（最終ブランチレビュー、
    段階D前半）。** `exit_fill()`のSTOP退出は当日の足（`bar.session`）で
    約定するが、MARKET退出・`entry_fill()`は翌営業日の足（`next_bar.session`）
    で約定する（段階B前半の規約）。同じ`volume`（出来高）は約定日の出来高で
    あるべきなので、STOP退出とMARKET退出/エントリーを同じ`VolumeBudget`
    インスタンスへ混ぜて渡すと、実際には異なる2つの営業日の出来高枠を
    1つとして扱ってしまう。日次ループ（段階D後半）は、`VolumeBudget`を
    「約定が起きる営業日」単位で作り直すこと（判断日単位ではない）。
    また`allow()`は銘柄ごとに**最初に渡された`volume`で枠を確定**し、
    以後その銘柄への呼び出しでは`volume`引数を無視する（同じ営業日内で
    出来高が変わることは無い前提のため）。
    """
