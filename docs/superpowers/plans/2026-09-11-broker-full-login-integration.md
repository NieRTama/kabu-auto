...
- **平日の無人自動実行に移行する前に**、kabu-auto本体を起動しているタスクスケジューラのタスクが「ログオン時のみ実行（対話セッションあり）」になっていることを確認する。WSL側 `login_kabustation.ps1` の SendKeys は対話デスクトップを必要とするため、非対話（サービス相当）実行だと動作しない（最終レビュー Recommendation #4 より）

### 2026-09-13 追記: 上記チェックリストを満たさないまま有効化しかけた事故

- 完全自動ログインの共有ロック二重起動バグ修正（`broker_process_lock.py`）と、
  ユーザー依頼によるスケジュール変更（平日06:45→08:30。`token_refresh` も
  8:30→8:35へ後ろ倒し）の作業中、`broker_full_login_enabled` を `true` に
  切り替えてしまった。その時点では上記チェックリスト（ユーザー立ち会いでの
  `full_login` 実行確認・実運用数日分のノイズ確認・タスクスケジューラの
  ログオン種別確認）を一つも満たしていなかった。
- 事後にこの計画書を読み返して発覚。`schtasks /query /tn "\kabu\kabu-auto" /xml`
  で実際に確認したところ `LogonType` は `S4U`（ログオン有無に関わらず実行）で、
  「ログオン時のみ実行（対話セッションあり）」ではなかった——本チェックリストが
  警告していたまさにその条件が未達だった。
- `config.yaml` の `broker_full_login_enabled` は一旦 `false` に戻した。
- **同日、ユーザーから「ログイン処理は自動とする。この方針は絶対とする」と
  明示指示があり、`broker_full_login_enabled: true` へ再度切り替えて確定した。**
  これによりチェックリストの1・2（人立ち会いでの実行確認・実運用数日分の
  ノイズ確認）は事後的に免除された運用判断となる。ただし3点目
  （タスクスケジューラのログオン種別）は技術的な前提条件であり方針決定とは
  独立して満たす必要がある——`LogonType: S4U` のままだと自動ログインの
  2段階認証入力（対話デスクトップが必要）が成功しない可能性が高い。
  実運用でログイン失敗が続くようなら、まずこのタスクスケジューラ設定
  （`InteractiveToken` への変更）を疑うこと。
