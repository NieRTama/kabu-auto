...
- 事後にこの計画書を読み返して発覚。`schtasks /query /tn "\kabu\kabu-auto" /xml`
  ...
  警告していたまさにその条件が未達だった。
- `config.yaml` の `broker_full_login_enabled` は `false` に戻し、上記3点を
  満たしてから改めて有効化することとした。次に有効化を検討する際は、まず
  タスクスケジューラのログオン種別を `InteractiveToken`（ログオン時のみ実行）
  へ変更できるか（対話セッションを維持したまま無人再起動後もタスクが動き続ける
  設定にできるか）を確認すること。
