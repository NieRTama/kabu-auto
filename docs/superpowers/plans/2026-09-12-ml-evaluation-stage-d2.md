...
- Produces:
  - `MarketData`（frozen dataclass）: `bars: dict`（symbol → 日付indexの**生**OHLCV。約定価格・必要資金・出来高）, `sectors: dict`, `features: dict`（symbol → **調整系列から作った特徴量フレーム**。`rule_score` と `feature_valid` を含む。既定は空）
  - `_row_for(md, symbol, session)` — 生の足に特徴量を重ねた判断用の1行。`feature_valid=False` の日は None
  - `_exit_score(md, symbol, session, exit_score_fn)` — 保有銘柄の売りスコア
...
- Produces:
  ...
  - `_drive_exits(state, md, session, policy_conf, costs, budget, exit_score_fn=None)` — 内部関数。保有ごとに `policy.step()` を回し、`STOP` は当日約定、`MARKET` は翌営業日へ繰り越す。**売りスコアを `Observation.score` へ渡し**、売り約定にも出来高の枠を掛ける。売れ残りは保有に残して意図を翌日へ持ち越す
  - `run_walkforward(..., exit_score_fn=None)` — 保有銘柄の売りスコア関数
...
**`decide` の契約:** `decide(session, rows, model, ctx) -> list[portfolio.Candidate]`。`rows` は `symbol -> その日の行（pd.Series）`で、**生の足に特徴量と `rule_score` を重ねたもの**（`_row_for()` の出力）。`md.features` が空なら生の足だけになる。`ctx` は `{"sectors": dict, "portfolio": Portfolio, "closes": dict}`。返した候補は**翌営業日の寄りで執行される**。

**候補の数量は前日終値ベースの見積りである。** 実際に買える株数は約定時点（翌朝の寄り）で決まる。`run_walkforward` は約定価格で現金・1銘柄上限・セクター上限・出来高枠を引き直し、足りなければ縮小するか見送って `rejected` に理由を残す（外部レビューR08）。
    ...
    for session in sessions:
        ...
        for order in pending_buys:
            ...
            result = execution.entry_fill_limited(
                obs, capped, costs, liquidity, volume=int(bar["volume"]))
            if result.unfilled_reason:
                rejected.append({"session": session, "symbol": order.symbol,
                                 "reason": result.unfilled_reason})
            if result.fill is None:
                continue
            # 出来高の枠も退出と共有する（外部レビューR09）
            granted = budget.allow(order.symbol, int(bar["volume"]),
                                   result.fill.quantity)
            if granted < pf.LOT_SIZE:
                rejected.append({
                    "session": session, "symbol": order.symbol,
                    "reason": "当日の出来高枠を退出で使い切ったため買えません"})
                continue
            state = pf.apply_buy(
                state, order.symbol, granted, result.fill.price,
...
- [ ] **Step 5b: 固定モデル実行を degraded にする**

`run_walkforward` のループ前に足す。
...
**この判定を入れる理由。** 段階Fのダッシュボード経路は `load_current()` の
戻り値を閉包に保持し、過去の全日付へ当てる書き方になっていた。学習期間と
検証開始日の前後関係を検査しないので、評価期間を学習済みのモデルでも
そのまま使える。`retrain` を渡さない実行を静かに通すと、その成績が
そのまま昇格判断に流れる。**degraded は「壊れた」という意味ではなく
「採否の根拠にできない」という意味**であり、診断実行はここに該当する。

- [ ] **Step 5c: 未来のデータが過去の予測へ届かないことを確かめる**
